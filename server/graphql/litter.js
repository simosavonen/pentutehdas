...
export const typeDefs = gql`
  ...
  extend type Mutation {
    ...
    deleteLitter(id: ID!): Litter
    toggleReservation(id: ID!): Litter
  }
...
export const resolvers = {
  Query: {
    allLitters: () =>
      Litter.find({}).populate(['dam', 'sire', 'breeder', 'reservations']),
  },
  Mutation: {
    addLitter: async (root, args, context) => {
      ...
      const populatedLitter = await Litter.findById(
        litter._id.toString()
    ...
    updateLitter: async (root, args, context) => {
      ...
      if (
        currentUser._id.toString() === litterToBeUpdated.breeder.toString() ||
        isAdmin
      ) {
    ...
    deleteLitter: async (root, args, context) => {
      ...
      const isAdmin = currentUser.role === 'admin'
      if (
        currentUser._id.toString() === litterToBeDeleted.breeder.toString() ||
        isAdmin
      ) {
      ...
      try {
        if (theLitter.reservations.includes(currentUser._id.toString())) {
          const updatedLitter = await Litter.findByIdAndUpdate(
            args.id,
            {
              reservations: theLitter.reservations.filter(
          ...
          const updatedLitter = await Litter.findByIdAndUpdate(
            args.id,
            {
              reservations: theLitter.reservations.concat(
